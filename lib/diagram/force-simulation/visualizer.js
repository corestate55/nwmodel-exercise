...
class ForceSimulationDiagramVisualizer extends BaseContainer {
  ...
  constructor() {
    ...
    const grpcUrl = `http://${window.location.hostname}:8080`
    this.grpcClient = new services.TopologyDataClient(grpcUrl, null, null)
  }

  _drawRfcTopologyData(topologyData, jsonName, alert, drawHookCallback) {
    /**
  ...
  _getDiagramDataViaGRPC(jsonName, alert, drawHookCallback) {
    const request = new messages.GraphRequest()
    request.setGraphName(messages.GraphName.FORCE_SIMULATION)
    request.setJsonName(jsonName)
    request.setTarget(alert.host)
    this.grpcClient.getDiagramData(request, {}, (error, response) => {
      if (error) {
        throw error
      }
      const topologyData = JSON.parse(response.getJson())
      this._drawRfcTopologyData(topologyData, jsonName, alert, drawHookCallback)
    })
  }

  _getDiagramDataViaREST(jsonName, alert, drawHookCallback) {
    json(`/api/graph/forceSimulation/${jsonName}`).then(
      topologyData => {
        this._drawRfcTopologyData(
          topologyData,
          jsonName,
          alert,
          drawHookCallback
  ...
  drawRfcTopologyData(jsonName, alert, drawHookCallback) {
    if (process.env.NODE_ENV === 'development') {
      this._getDiagramDataViaGRPC(jsonName, alert, drawHookCallback)
    } else {
      this._getDiagramDataViaREST(jsonName, alert, drawHookCallback)
    }
  }
