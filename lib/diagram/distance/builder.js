...
class DistanceDiagramBuilder extends DiagramBase {
  ...
  _makeTpTpLinkLine(nodes, links) {
    ...
    this.linkGroupSelection
      .selectAll('line.inter-nodes')
      .data(links)
      .enter()
      .append('line')
      .attr('id', d => d.path)
      .attr('class', d => this.classStringFrom(d, 'dist inter-nodes'))
      .attr('x1', d => nodePos(d.sourceNodePath).cx)
      .attr('y1', d => nodePos(d.sourceNodePath).cy)
      .attr('x2', d => nodePos(d.targetNodePath).cx)
      .attr('y2', d => nodePos(d.targetNodePath).cy)
  }
